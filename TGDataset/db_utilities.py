...
def insert_channel(new_channel, db_name='TGD4'):
...
#   - db_name -> specify the name of the collection in MongoDB
def get_text_messages_by_id_ch(id_channel, db_name='TGD4'):
    with MongoClient(uri) as client:
        db = client[db_name]
        fs = gridfs.GridFS(db)
...
#   - db_name -> specify the name of the collection in MongoDB
def get_channel_by_id(id_channel, db_name='TGD4'):
    ch = {}
    with MongoClient(uri) as client:
        db = client[db_name]
...
#   - db_name -> specify the name of the collection in MongoDB
def get_channel_by_username(username, db_name='TGD4'):
    ch = {}
    with MongoClient(uri) as client:
        db = client[db_name]        
...
#   - db_name -> specify the name of the collection in MongoDB
def get_channels_by_ids(ids_channels, db_name='TGD4'):
    chs = []
    with MongoClient(uri) as client:
        db = client[db_name]
...
# Parameters:
#   - db_name -> specify the name of the collection in MongoDB
def get_channel_ids(db_name='TGD4'):
    with MongoClient(uri) as client:
        db = client[db_name]
...
def import_channels_to_mongoDB(db_name='TGD4', root_directory='/media/teun/Hard Driver/TxMM/TGDataset_1/public_db', fast_mode=False):
...
# Parameters:
#   - db_name -> specify the name of the collection in MongoDB
def get_other_channels_references(db_name='TGD4'):
...
if __name__ == '__main__':
    import_channels_to_mongoDB('TGD1')
    print(get_channel_ids("TGD1"))
